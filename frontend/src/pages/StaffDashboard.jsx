...
export default function StaffDashboard() {
  ...
  const loadJobs = async () => {
    try {
      ...
      setJobs(data);
      // Synchronize analytics and badges in background
      fetchEarningsAnalytics().then((aData) => setAnalytics(aData)).catch(() => {});
    } catch (e) {
  ...
  const handleConfirmReject = (jobId) => {
    const reason = rejectReason;

    // 1. Instantly close modal (0ms UI feedback)
    setRejectingJob(null);

    // 2. Optimistically update local jobs list immediately (0ms)
    setJobs((prevJobs) =>
      prevJobs.map((j) =>
        j.job_id === jobId
          ? {
              ...
              staff_notes: `Cancelled: ${reason}`,
            }
          : j
      )
    );

    // 3. Optimistically update analytics counters immediately (0ms)
    setAnalytics((prev) => {
      if (!prev) return prev;
      return {
        ...prev,
        summary: {
          ...prev.summary,
          rejected_orders: (prev.summary?.rejected_orders || 0) + 1,
          queued_orders: Math.max(0, (prev.summary?.queued_orders || 1) - 1),
        },
      };
    });
    ...
    rejectJob(jobId, reason)
      ...
      .catch((err) => {
        ...
        alert("Could not cancel order: " + (err.message || "Network error"));
        loadJobs();
      });
  };
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-6">
            ...
            <div className="bg-white p-6 rounded-3xl border border-slate-200/80 shadow-sm space-y-4">
              ...
              <div className="pt-2 border-t border-slate-100 flex items-center justify-between text-xs text-slate-500">
                <span className="flex items-center space-x-1">
                  ...
                  <span>Digital Collections Ratio</span>
                </span>
                <span className="font-extrabold text-slate-800">
                  {totalRevDisplay > 0
                    ? Math.round(((analytics?.payment_breakdown?.razorpay?.amount ?? 0) / totalRevDisplay) * 100)
                    : 0}% Online
                </span>
              </div>
            </div>
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-200 text-slate-400 font-bold uppercase tracking-wider text-[10px]">
                  <th className="py-3 px-3">Order Token</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {paymentRecords.map((p, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3 px-3 font-black text-slate-900">₹{(p.amount_inr || 0).toFixed(2)}</td>
                    <td className="py-3 px-3">
                    ...
                    <td className="py-3 px-3 text-slate-500 text-[11px]">{p.created_at || "—"}</td>
                  </tr>
                ))}
            ...
            <table className="w-full text-left text-xs border-collapse">
              <thead>
                <tr className="border-b border-slate-200 text-slate-400 font-bold uppercase tracking-wider text-[10px]">
                  ...
                  <th className="py-3 px-3">Print Specifications</th>
                  <th className="py-3 px-3">Amount (₹)</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {printoutRecords.map((r, idx) => (
                  <tr key={idx} className="hover:bg-slate-50/70 transition-colors">
                    ...
                    <td className="py-3 px-3 max-w-[180px] truncate font-medium text-slate-700" title={r.file_name}>
                      {r.file_name || "Document.pdf"}
                    </td>
                    <td className="py-3 px-3">
                      <span className="inline-block px-2.5 py-1 bg-slate-100 text-slate-800 rounded-lg text-[11px] font-semibold">
                        {r.specs || `${r.pages || 1} pgs × ${r.copies || 1} • ${(r.color_mode || "bw").toUpperCase()}`}
                      </span>
                    </td>
                    <td className="py-3 px-3 font-black text-slate-900">₹{(r.total_amount_inr || 0).toFixed(2)}</td>
                    <td className="py-3 px-3">
                      <span
                        className={`px-2 py-0.5 rounded-full text-[10px] font-bold uppercase ${
                          r.status === "READY" || r.status === "COMPLETED" || r.status === "ready" || r.status === "completed"
                    ...
                    <td className="py-3 px-3">
                      {(r.payment_status || "").toLowerCase() === "paid" ? (
                        <span className="text-emerald-700 font-bold text-[10px] bg-emerald-50 px-2 py-0.5 rounded">PAID</span>
                      ) : (
                        <span className="text-amber-700 font-bold text-[10px] bg-amber-50 px-2 py-0.5 rounded">UNPAID</span>
                      )}
                    </td>
                    <td className="py-3 px-3 text-slate-500 text-[11px]">{r.created_at || "—"}</td>
                  </tr>
                ))}
      ...
      {/* Reject / Cancel Order Reason Modal */}
      {rejectingJob && (
          ...
          <div className="bg-white rounded-2xl max-w-md w-full border border-slate-200 shadow-2xl p-6 space-y-4">
            <div className="flex items-center justify-between">
              <h3 className="font-bold text-sm text-slate-900 flex items-center space-x-2">
                <XCircle className="w-4 h-4 text-rose-600" />
                <span>Reject / Cancel Order #{rejectingJob}</span>
              </h3>
              <button
                type="button"
            ...
            <p className="text-xs text-slate-500">
              Select a reason for cancelling or rejecting this print request. The order status will be updated immediately.
            </p>

            <div className="space-y-2">
              {[
                "Cancelled by customer / student request",
...
function JobCard({ job, actionButton, onReject }) {
  ...
  return (
    <div className="bg-white p-3.5 rounded-xl border border-slate-200/90 shadow-sm hover:shadow-md transition-shadow space-y-2.5">
      ...
      <div>
        <div className="flex items-center justify-between">
          <span className="text-xs font-bold text-slate-800 truncate">{job.sender_name || "Student"}</span>
          <span className="text-[10px] text-slate-400 font-medium truncate max-w-[120px]">
            {job.file_name || "Document"}
          </span>
        </div>
        {job.message_text && !job.message_text.includes("copies") && !job.message_text.startsWith("/") && (
